...
python3 /home/pto/lean/repo/experiments/models/qwen_explicit_block_probe.py --model-path \"\$MODEL_REF\" --layer-idx \"$LAYER_IDX\" --device \"$DEVICE\" --prompt \"$PROMPT\" --prompt-format \"$PROMPT_FORMAT\" --max-prefill-tokens \"$MAX_PREFILL_TOKENS\" \$EXTRA_ARGS"

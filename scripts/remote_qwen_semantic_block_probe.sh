...
python3 /home/pto/lean/repo/experiments/models/qwen_semantic_block_probe.py --model-path \"\$MODEL_PATH\" --layer-idx \"$LAYER_IDX\" --device \"$DEVICE\" --prompt \"$PROMPT\" --prompt-format \"$PROMPT_FORMAT\" --max-prefill-tokens \"$MAX_PREFILL_TOKENS\" --page-size \"$PAGE_SIZE\" \$EXTRA_ARGS"

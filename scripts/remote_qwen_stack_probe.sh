...
python3 /home/pto/lean/repo/experiments/models/qwen_explicit_stack_probe.py --model-path \"\$MODEL_PATH\" --num-layers \"$NUM_LAYERS\" --device \"$DEVICE\" --prompt \"$PROMPT\" --prompt-format \"$PROMPT_FORMAT\" --max-prefill-tokens \"$MAX_PREFILL_TOKENS\" \$EXTRA_ARGS"
